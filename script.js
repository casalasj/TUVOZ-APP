...
// --- 2. SÍNTESIS DE VOZ (Web Speech API - Gratuita, sin API Key) ---

// Prioridades de voces en español: Microsoft Neural > Google > otras
// Se prefieren voces en español de Latinoamérica (es-MX, es-US, es-CL, es-AR) y España
const VOICE_LANG_PRIORITY = ['es-CL', 'es-MX', 'es-US', 'es-AR', 'es-CO', 'es-ES', 'es'];

let allVoices = [];
let currentGenderFilter = 'A'; // se sincroniza con setGender()

// Nombres de voces Microsoft que conocemos por género
const MICROSOFT_FEMALE_KEYWORDS = ['Paloma','Elena','Renata','Salome','Camila','Maria','Ximena','Abril','Dalia','Lorena','Isabella','Fernanda','Francisca','Catalina','Laura','Pilar','Elvira','Alba','Jimena'];
const MICROSOFT_MALE_KEYWORDS   = ['Jorge','Raul','Liberto','Andres','Tomas','Felipe','Gerardo','Pelayo','Alvaro','Arnau','Sergio'];
// Palabras genéricas en nombres de voces del sistema
const GENERIC_FEMALE_KEYWORDS = ['Female','Mujer','femenin','woman','girl'];
const GENERIC_MALE_KEYWORDS   = ['Male','Hombre','masculin','man','boy'];

function guessVoiceGender(voice) {
    const name = voice.name;
    // Microsoft Neural naming: "Microsoft Dalia Online (Natural) - Spanish (Mexico)"
    for (const kw of MICROSOFT_FEMALE_KEYWORDS) if (name.includes(kw)) return 'F';
    for (const kw of MICROSOFT_MALE_KEYWORDS)   if (name.includes(kw)) return 'M';
    for (const kw of GENERIC_FEMALE_KEYWORDS)   if (name.toLowerCase().includes(kw.toLowerCase())) return 'F';
    for (const kw of GENERIC_MALE_KEYWORDS)     if (name.toLowerCase().includes(kw.toLowerCase())) return 'M';
    return 'U'; // Unknown
}

function getVoiceLangScore(voice) {
    for (let i = 0; i < VOICE_LANG_PRIORITY.length; i++) {
        if (voice.lang.startsWith(VOICE_LANG_PRIORITY[i])) return i;
    }
    return 999;
}

function populateVoiceSelect(genderFilter) {
    const select = document.getElementById('voiceSelect');
    const loadingMsg = document.getElementById('voiceLoadingMsg');
    ...
    // Ordenar: primero por prioridad de locale, luego Microsoft Neural primero
    voices.sort((a, b) => {
        const langDiff = getVoiceLangScore(a) - getVoiceLangScore(b);
        if (langDiff !== 0) return langDiff;
        // Microsoft Neural antes que otras
        const aMs = a.name.includes('Microsoft') ? 0 : 1;
        const bMs = b.name.includes('Microsoft') ? 0 : 1;
        return aMs - bMs;
    });

    // Construir opciones con género detectado
    select.innerHTML = '';
    ...
    voices.forEach(voice => {
        ...
        if (usedNames.has(voice.name)) return; // evitar duplicados
        usedNames.add(voice.name);

        // Aplicar filtro de género
        if (genderFilter !== 'A' && gender !== 'U' && gender !== genderFilter) return;
        ...
        const localeLabel = voice.lang.toUpperCase().replace('-', '_');
        const genderIcon  = gender === 'F' ? '♀' : gender === 'M' ? '♂' : '◈';
        opt.textContent = `${genderIcon} ${displayName} [${localeLabel}]`;

        select.appendChild(opt);
        if (!firstVisible) firstVisible = opt;
    });

    // Si con filtro de género no hay voces, mostrar todas
    if (select.options.length === 0) {
        populateVoiceSelect('A');
        return;
    }
...
function loadVoices() {
    ...
    if (allVoices.length > 0) {
        populateVoiceSelect(currentGenderFilter);
    }
}

// La API dispara 'voiceschanged' cuando las voces están listas (especialmente en Chrome)
window.speechSynthesis.onvoiceschanged = loadVoices;
loadVoices(); // Intento inmediato (funciona en Firefox/Safari)

// Referencia a la utterance activa para poder cancelarla
let activeUtterance = null;

function speakText() {
    const textInput = document.getElementById('textToSpeak').value.trim();
    if (!textInput) return;
    ...
    btnHablar.innerHTML = '<i class="fa-solid fa-volume-high fa-beat"></i> HABLANDO...';
    btnHablar.disabled = true;
    activeUtterance = utterance;

    utterance.onstart = () => addToHistory(textInput);

    utterance.onend = () => {
        clearText();
        btnHablar.innerHTML = '<i class="fa-solid fa-volume-high"></i> HABLAR';
    ...
    // Workaround: Chrome pausa el TTS en segundo plano después de ~15s
    // Este keepalive lo previene
    if (typeof _ttsKeepAlive !== 'undefined') clearInterval(_ttsKeepAlive);
...
function setGender(gender) {
    ...
    currentGenderFilter = gender; // sincroniza con el selector de voces
    ...
    // Repoblar selector de voces filtrando por género
    populateVoiceSelect(gender);
